...
/**
 * a container contains key<String>-value<String>pair TC has no construct
 * method, we can only instance it by the static method
 * <code>KeyValuePair parseLine(String str)</code>
 * 
 * @see parseLine
 * @author beiming
 * 
...
public class KeyValuePair<T, V> {
	...
	/**
	 * we can instance TC by this static method. this method will parse a line
	 * constructed as "key=value" it will first trim this line,if the line if
	 * not null
	 * 
	 * @param str
	 *            the line need to be parsed
	 * @return if str in good format,will return a <code>KeyValuePair</code>
	 *         instance. if str is null starts with "#" contains no "=" starts
	 *         with "=" then null will returned
	 */
	public static KeyValuePair<String, String> parseLine(String str) {
		...
		if (pos == -1)
			return null;
		kv = new KeyValuePair<String, String>(str.substring(0, pos).trim(), str
	...
	/**
	 * split line with {'\n','\t',' ', '\r','��'}. First the line will be
	 * trimed. Key and Value will also be trimed.
	 * 
	 * @param str
